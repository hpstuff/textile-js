...
"use strict";
  ...
  var re_block          = re.compile( /^([:blocks:])/ )
    ...
    , re_quote          = re.compile( /^((?:[\t ]*[\>]+[:pba_attr:] .+?(?:\r?\n|$))+)(\s*\r?\n)?/ )
    , re_quote_line     = /^([\>])(.+?)(\r?\n|$)/
    , re_list_item      = /^([\#\*]+)(.+?)(\r?\n|$)/
    , re_deflist        = /^((?:- (?:[^\n]\n?)+?)+:=(?: *\n[^\0]+?=:(?:\n|$)|(?:[^\0]+?(?:$|\n(?=\n|- )))))+/
    , re_deflist_item   = /^((?:- (?:[^\n]\n?)+?)+):=( *\n[^\0]+?=:\s*(?:\n|$)|(?:[^\0]+?(?:$|\n(?=\n|- ))))/
  ...
  function parse_quote(src, options){
    src = ribbon( src.replace( /(^|\r?\n)[\t ]+/, '$1' ) );
    var pad = function ( n ) {
          var s = '\n';
          while ( n-- ) { s += '\t'; }
    ...
    while ( (m = re_quote_line.exec( src )) ) {
        stack.push (m[2]+m[3]);
        src.advance( m[0] );
    }
    return [ 'blockquote', {}, '\n' ].concat( parse_blocks( stack.join("\r\n"), options) ).concat(['\n']);
  ...
  function parse_blocks ( src, options ) {
    ...
    while ( src.valueOf() ) {
      ...
      if ( (m = re_quote.exec( src )) ) {
        src.advance( m[0] );
        list.add( parse_quote( m[0], options ) );
      }
